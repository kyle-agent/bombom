...
| `/place?path=<zone>` | `place.html` | **존 화면** — 좌측 배치 트리 + 멀티랙 실장도. **✏ 편집 토글**로 보기↔배치 전환 |
| `/zone?path=` | `zone.html` | `/place`로 리다이렉트(구 링크 호환) |
| `/summary` | `summary.html` | **투자 리포트** — 오퍼링/리전/존/랙타입별 선택 집계(CAPEX·서버·장비·랙) |
...
**배치 UX 요약(`/place` 편집 모드):** 좌상단 = 배치된 목록(랙→카테고리→장비×수량, 접힘 트리),
좌하단 = 팔레트. `＋ 장비 추가`(후보풀 검색) → `＋담기` → 팔레트 칩 **클릭 → 랙 빈 U칸 클릭**으로
배치(또는 드래그). 블록 **드래그=이동**, **⧉=팔레트에 담기(다른 랙 복제)**, **Alt+드래그=복사**,
**✕=제거**. 휠=줌, 빈 곳 드래그=팬. `💾 저장`은 랙별로 `PUT /api/rack` 커밋.
